...
import { PieData } from '../../../models/charts/pie.chart.model';
import { LineHorizantalChartData } from '../../../models/charts/line.chart.model';
...
export class ReceiptService {
  ...
  processReceiptsAIAssissted(file: File): Observable<{message: string, document_details: Receipt}> {
    ...
    return this.http.post<{message: string, document_details: Receipt}>(url, formData);
  }

  storeReceiptsAIAssissted(file: File,total:number,sub_total:number,tax:number,company_name: string,
    vendor:string,purchased_at:string,vendor_address:string,customer_name:string,
    invoice_id:string,spend_type:string
 ...
 /**
   ...
   */
  getListOfReceipts(): Observable<{ receipts: Receipt[], msg: string }> {
    const url = `${this.apiUrl}${ApiConstants.GET_LIST_OF_RECEIPTS}`;

    console.log(url);

    return this.http
      .get<{ receipts: Receipt[], msg: string }>(url);
  }
